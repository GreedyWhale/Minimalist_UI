...
export default class MDatePicker extends Vue {
  ...
  onInput(event: string): void {
    ...
    if (rungReg.test(event) || slashReg.test(event)) {
      ...
      const isEffectiveDate = Object.keys(selectedDate).every(
        (key: string) => !!selectedDate[key]
      );
      if (!isEffectiveDate) {
        return;
      }
      if (
        selectedDate.year !== this.currentDate.year ||
        selectedDate.month !== this.currentDate.month
      ) {
        this.dateList = this.calender.getDateList(
          selectedDate.year,
          selectedDate.month
