...
export default class MCollapse extends Vue {
  ...
  @Prop({ type: [String, Number, Array], default: '' }) private activeName!: string | number | any[];;
  // data
  activeNamesCopy: any[] = [];
  mounted(): void {
  ...
  getActiveNamesCopy(): any[] {
    let names = []
    if(this.activeNames) {
      names = this.activeNames instanceof Array ? [...this.activeNames, ...this.activeNamesCopy] : [this.activeNames, ...this.activeNamesCopy]
    }
    if(this.activeName) {
      names = this.activeName instanceof Array ? [...this.activeName, ...this.activeNamesCopy] : [this.activeName, ...this.activeNamesCopy]
    }
  ...
  listenChangeActiveName(): void{
    this.eventBus.$on('changeActiveName', (name: string, isOpen: boolean) => {
      const activeNames: any[] = JSON.parse(JSON.stringify(this.activeNamesCopy));
      if(isOpen){
        // 需要关闭
