  ...
  <div class="m-pagination">
    <ol v-if="displayPage.length" class="pager-list">
      ...
      <li
        ...
        :key="index"
        :class="{'no-border': item.useEllipsis, active: item.value === currentPage}"
        :title="item.title"
...
export default class MPagination extends Vue {
  ...
  get displayPage(): number[]  {
    ...
    totalList = totalList.concat(...this.setTotalListWithoutFirstAndLast())
    .filter((value: any) => value > 0 && value <= this.total)
    .sort((a:number, b:number) => a - b)

    totalList = [...new Set(totalList)];
    totalList = totalList.reduce((acc: any[], cur: number, idx: number, src: number[]) => {
      acc.push({value: cur})
      if(src[idx + 1] !== undefined && (src[idx + 1] - cur) > 1) {
        acc.push({
          useEllipsis: true,
          direction: cur < this.currentPage ? 'left' : 'right',
          title: cur < this.currentPage ? `向前 ${this.step} 页` : `向后 ${this.step} 页`
        })
      }
      return acc;
    }, [])
  ...
  onClick(item: any, index: number) {
    ...
    if(item.useEllipsis) {
      current = item.direction === 'left' ? current - this.step : current + this.step;
    }
  ...
  onMouseEnter(item: any, index: number) {
    ...
    if(item.useEllipsis) {
      ...
      icon = item.direction === 'left' ? 'double-arrow-left' : 'double-arrow-right'
    }
