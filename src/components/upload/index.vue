...
export default class MUpload extends Vue {
  ...
  @Prop({ type: Function }) private beforeRemove!: (file: any, fileList: any[]) => any;
  @Prop({ type: Function }) private beforeUpload!: (file: any, fileList: any[]) => any;
  ...
  handleRemove(file: any): void {
    const doRemove = () => {
      ...
      const successed = fileList.filter((fileItem: any) => fileItem.status === 'success');
  ...
  canUpload(file: File): boolean {
    ...
    if (this.size) {
      if (size > this.size * 1024) {
        ...
        const err: any = new Error(`上传文件大小：${size / 1024} kb，超出指定大小：${this.size} kb`);
        this.handleError(err ,file);
        return false;
      }
    }
